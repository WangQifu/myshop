		...
		function addtocart()//加入购物车
		{
			...
			if(!$getUser) //用户没有登录情况，计入cookie
			{
				...
				{ 
					...
					//setcookie(User_Cart_CookieKey,"",time()-3600*24,"/");
				}
				else //没有加入过购物车，则初始化
				{
				    $value = myCrypt(serialize(array($getPid)), User_Cart_CryptKey);
